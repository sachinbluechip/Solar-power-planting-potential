...
cred = credentials.Certificate('dashboard-9cafb-firebase-adminsdk-tpiop-7ba9cf35b9.json')  # Replace with your Firebase service account path
...
def upload_file():
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            # Generate random data_image.png"
            import random
            random_data = {
                "location": random.choice(["Location A", "Location B", "Location C"]),
                "solar_score": solar_potential_score,
                "brightness": avg_brightness,
                "time_of_day": firestore.SERVER_TIMESTAMP
            }

            # Save random data to Firestore
            doc_ref = db.collection('solar_data').document()
...
def dashboard():
    ...
    # random_data = {
    #     "location": random.choice(["Location A", "Location B", "Location C"]),
    #     # "solar_score": random.randint(100, 200),
    #     "solar_score": solar_potential_score,
    #     # "brightness": random.uniform(0, 255),
    #     "brightness": avg_brightness,
    #     "time_of_day": random.randint(6, 18)
    # }
